...
| Demo initial load | The demo now loads seven non-home section views on demand, following the app dashboard's existing lazy-section pattern. Root section URLs also restore the selected section after refresh. | The entry script fell from 1,349 to 1,112 KiB (about 18%). A local production preview rendered all seven sections through navigation and direct URLs without console errors. |
