...
must remain available for queued work to start. If the runner is offline, the accepted command
waits. If task creation fails, the coordinator removes its claim so a later run can retry.
