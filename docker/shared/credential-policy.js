// Generated by scripts/build-runtime-policies.mjs. Edit packages/core/src/credential-policy.ts.
...
function isCredentialFieldName(value) {
	...
	return normalized.indexOf("token") >= 0 || normalized.indexOf("password") >= 0 || normalized.indexOf("passwd") >= 0 || normalized.indexOf("passcode") >= 0 || normalized.indexOf("jwt") >= 0 || normalized.indexOf("secret") >= 0 || normalized.indexOf("credential") >= 0 || normalized === "key" || normalized === "sig" || normalized === "pin" || normalized === "code" || normalized === "authorization" || normalized === "auth" || normalized === "authsig" || normalized.indexOf("signature") >= 0 || normalized === "bearer" || normalized === "accesskey" || normalized === "accesscode" || normalized === "privatekey" || normalized.slice(Math.max(0, normalized.length - 6)) === "apikey" || normalized.indexOf("api") === 0 && normalized.slice(Math.max(0, normalized.length - 3)) === "key";
}
...
export default { isCredentialFieldName, isCredentialBearingUrl };
