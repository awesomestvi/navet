...
export function WindOverlaySvg({
  effectsQuality,
  size,
}: {
  ...
  const windLayerId = `${useId()}-wind`;
  const motionClassName =
    effectsQuality === 'high'
      ? 'motion-safe:animate-[navet-weather-cloud-loop_11s_linear_infinite] motion-safe:will-change-transform [transform-box:view-box]'
      : effectsQuality === 'medium'
        ? 'motion-safe:animate-[navet-weather-cloud-loop_18s_linear_infinite] motion-safe:will-change-transform [transform-box:view-box]'
        : undefined;
  ...
  return (
    ...
    >
      ...
      <g className={motionClassName}>
        <use href={`#${windLayerId}`} />
        {motionClassName ? <use href={`#${windLayerId}`} transform="translate(100 0)" /> : null}
      </g>
    </svg>
  );
}
