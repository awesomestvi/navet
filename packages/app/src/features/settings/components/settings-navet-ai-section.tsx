...
import { AudioLines, CameraOff, FileText, KeyRound, Power, Telescope, Trash2 } from 'lucide-react';
...
export function SettingsNavetAiSection({ controller }: { controller: SettingsSectionController }) {
  ...
  const [turnOffConfirmationOpen, setTurnOffConfirmationOpen] = useState(false);
  ...
  return (
    <SettingsSectionShell
      id="navet-ai"
      icon={Telescope}
      title={t('navetAi.settings.sectionTitle')}
        ...
        <SettingsItem
          ...
          description={
            state.settings.enabled
              ? t('navetAi.settings.resetDescription')
              : t('navetAi.settings.disabledDescription')
          }
          styles={controller.styles}
        >
          {state.settings.enabled ? (
            <AlertDialog open={turnOffConfirmationOpen} onOpenChange={setTurnOffConfirmationOpen}>
              <Button
                variant="destructive"
                size="small"
                disabled={loading}
                leading={<Power className="h-4 w-4" aria-hidden="true" />}
                onClick={() => setTurnOffConfirmationOpen(true)}
              >
              ...
              <AlertDialogContent>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction
                    ...
                    onClick={() => {
                      void reset();
                      setTurnOffConfirmationOpen(false);
                    }}
                  >
                    {t('navetAi.settings.resetAction')}
                  </AlertDialogAction>
            ...
            >
              {t('navetAi.settings.enableAction')}
            </Button>
          )}
        </SettingsItem>
      ) : null}
              ...
              >
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p
                      className={`truncate text-base font-semibold ${controller.styles.textColor}`}
                    >
