...
/**
 * Debounces async device commands with in-flight tracking.
 *
...
export function useCommandQueue<T>(
  ...
  const flush = useCallback(() => {
    ...
    // Senders own user-facing error feedback; failures still release the queue.
    try {
      void sendRef.current(value).then(complete, complete);
    } catch {
      complete();
    }
  }, []);
